...
WORKFLOW = 'Fetch'
WORKER = 'Fetch'
SLOTS = int(os.environ.get('WORKER_SLOTS_FETCH', '30'))
i = 0


DatasetFetchWorkflow = hatchet.workflow(
    name=WORKFLOW, on_events=['dataset:fetch'],
    concurrency=concurrency(WORKFLOW_LIMIT), input_validator=WorkflowInput
)

@DatasetFetchWorkflow.task(
    schedule_timeout=SCHEDULE_TIMEOUT,
    execution_timeout=STEP_TIMEOUT,
    retries=STEP_RETRIES
)
def download_data(args: WorkflowInput, context: Context) -> dict:
    global i
    task_id = uuid.uuid4()
...
def run():
    worker = hatchet.worker(
        WORKER, slots=SLOTS,
        workflows=[DatasetFetchWorkflow]
    )
    set_signal_handler()
    worker.start()
