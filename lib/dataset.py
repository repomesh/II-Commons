...
def init(name, i2d=False, materialized=False):
    ...
    dataset = get_dataset(name, materialized=materialized)
    dataset.init()
    ...
    def get_s3_key(meta):
        ...
        return os.path.join(
            RULES[name].get('s3_path', S3_PATH), f"{meta['hash']}.jpg"
        )
...
def get_datasets():
    resp_tb = query("""SELECT table_name
        FROM information_schema.tables
        WHERE table_schema = 'public'
        AND table_type = 'BASE TABLE'
        ORDER BY table_name""")
    return sorted([table['table_name'][3:] for table in resp_tb])
